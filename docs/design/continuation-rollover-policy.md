...
identical. The final inventory read occurs inside a consistent deferred SQLite
transaction after the new revision is computed but immediately before the
pointer CAS; no writer lock is reserved during that external read. The pointer
CAS, immutable snapshot/rows, and receipt event then commit as one transaction,
and both observation digests are bound to that receipt. Runtime generation is
derived from the observed terminal plus the exact session/thread identity and
must also equal the canonical generation when one exists. A changed endpoint,
route, session, terminal, sequence, descendant set, owner/fence, runtime
...
| #23 | `league rollover refresh-bindings --operation-id OP --refresh-id ID --squad-id SQUAD --predecessor-agent-id OLD --successor-agent-id NEW --expected-rollover-version N --expected-snapshot-version N --expected-snapshot-digest DIGEST --expires-at TIME --at TIME [--mode-action ACTION --expected-mode-goal-version N]` | Replace only an expired snapshot for the exact already-switched operation after one full canonical read and two identical Herdr observations. The mutating command is `live_reconcile` protected: mode authority binds the complete canonical target scope and settled retries are effect-free; omitting mode inputs preserves direct explicit-authority compatibility. The final observation runs inside the deferred transaction immediately before pointer CAS without reserving the writer lock; the prior revision and owner switch remain immutable. Partial progress requires an exact prior descendant receipt and becomes a terminal marker without removing the original row. |
