...
- [#11: Generalize guarded teardown across task classes](https://github.com/Vinosaamaa/league-of-orchestrator/issues/11) — PR #30 established proof-first recoverable plans; this successor adds canonical SQLite-backed production execution, exact shared-lease release, blocked/final receipts, completed-action revalidation, and rollover-predecessor drain without performing live teardown
...
- [#23: Build an isolated League acceptance sandbox and reversible cutover harness](https://github.com/Vinosaamaa/league-of-orchestrator/issues/23) — this continuation integrates the repository-local lifecycle, read-only explicit-binding shadow, staged-inactive League/watcher bundle, source/staged/current-install manifests, backup/restore rehearsal, isolated runtime-contract canaries, supervision measurements, one real disposable Codex/Herdr report-to-teardown E2E, and one deterministic no-apply mutation manifest; the separately authorized global switch and live smoke remain open
...
- [#40: Require repository-owned durable artifacts before teardown](https://github.com/Vinosaamaa/league-of-orchestrator/issues/40) — merged repository-local v9 records expected repository artifacts and exact merged-publication receipts, then refuses cleanup while publication is unresolved; final real disposable acceptance remains #23-owned
- [#81: Implement durable scoped autonomous delivery mode](https://github.com/Vinosaamaa/league-of-orchestrator/issues/81) — this candidate adds immutable grant revisions, exact bounded action/repair receipts, six stable mode commands, Shotcaller-only external ownership, duplicate-preflight issue selection with concurrency fencing and prior linkage, issue-bound `assign run`, and generic visible-Champion routing for durable work; merge/install/live acceptance remain separate gates
- [#90: Stop League releases from replacing the universal agent guide](https://github.com/Vinosaamaa/league-of-orchestrator/issues/90) — this candidate makes terminal-environment-toolkit issue #45 the sole universal-guide owner, packages only the League orchestration supplement, and refuses the universal target before staged file or pointer mutation
