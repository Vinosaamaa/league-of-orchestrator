...
class RuntimeLifecycle:
    ...
    def _bound(self, binding_id: str) -> tuple[dict[str, Any], Any, Any, OpaqueIdentity, OpaqueIdentity]:
        ...
        return binding, harness, backend, session, endpoint

    def prompt(self, binding_id: str, prompt: str) -> AdapterReceipt:
        _, harness, backend, session, endpoint = self._bound(binding_id)
        harness.contract.require("prompt", HARNESS_CAPABILITIES)
        backend.contract.require("input", BACKEND_CAPABILITIES)
        return backend.input(endpoint, harness.prompt(session, prompt))

    def wake(self, binding_id: str, event: str) -> AdapterReceipt:
        _, harness, backend, session, endpoint = self._bound(binding_id)
        harness.contract.require("hook", HARNESS_CAPABILITIES)
        backend.contract.require("input", BACKEND_CAPABILITIES)
    ...
    def interrupt(self, binding_id: str) -> AdapterReceipt:
        _, harness, backend, session, endpoint = self._bound(binding_id)
        harness.contract.require("interrupt", HARNESS_CAPABILITIES)
        backend.contract.require("input", BACKEND_CAPABILITIES)
    ...
    def status(self, binding_id: str) -> str:
        ...
        return harness.status(session, backend.inspect(endpoint))

    def guarded_exit(self, binding_id: str, expected_version: int, at: str) -> dict[str, Any]:
        binding, harness, backend, session, endpoint = self._bound(binding_id)
        ...
        if observation.state not in {"closed", "missing"}:
            ...
            backend.input(endpoint, harness.exit(session))
        after_exit = backend.inspect(endpoint)
        ...
        if after_exit.state != "missing":
            backend.close(endpoint)
        closed = backend.inspect(endpoint)
