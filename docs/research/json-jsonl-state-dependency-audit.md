...
| C1 | `~/.agents/league-champions.json` | Herdr launch reserve; teardown/launch rollback release; other creation remains external | Launch/preflight, teardown, tests, guide | Canonical visible allocation | No schema version; `available` plus `in_use`; an in-use assignment may bind `record` to its absolute record directory or legacy `status.json`; current pending entries add `task_id` and `pending`; new-launch teardown requires the exact directory locator; pool, runtime, pending attempt, and record publication are not one transaction | `callsigns`, leases, agent incarnations, and events; preserve active legacy record locators in named agent metadata, model pending locators as `launch_attempts.record_locator`, and refuse malformed or identity-stale locators; issue #13 owns cooldown/history policy |
