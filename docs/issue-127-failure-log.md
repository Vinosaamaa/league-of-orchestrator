...
| CLI proof fixture | The synthetic CLI E2E returned an agent-list shape for the newly required pane process inspection. | Updated only the fake Herdr command to return the supported structured `not_found` result for its absent pane. | Synthetic temporary state only. |
