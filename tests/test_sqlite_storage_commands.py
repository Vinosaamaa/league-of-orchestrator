...
def test_launcher_help_and_schemas() -> None:
    ...
    assert (
        "{storage,agent,callsign,shotcaller,rollover,delivery,project,roster,evidence,artifact,report,squad,task,runtime,skill,routing,resource,cleanup,request,assign,hook,mode,help,acceptance}"
        in launcher.stdout
    )
    parser = cli._parser()
