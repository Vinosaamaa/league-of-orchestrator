...
- [#8: Support guarded disposable Shotcaller handoff](https://github.com/Vinosaamaa/league-of-orchestrator/issues/8) — repository-local v6 state now has a bounded public-safe startup context and one configured-provider high-level command over the stable-Squad CAS switch, immutable Champion snapshot, exact successor acknowledgement, intake fencing, zero-obligation drain, and one owner event/outbox; live Codex↔Cursor acceptance remains #23-gated
