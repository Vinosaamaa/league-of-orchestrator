...
def test_schema_and_command_inventory() -> None:
    ...
    assert version.stdout.strip() == "league 0.2.34"
...
def test_existing_release_identity_precedes_install_mutation(root: Path) -> None:
    for name, relative in (
        ("release", Path("stage-prefix/releases/0.2.34")),
        ("bundle", Path("release-bundle/0.2.34")),
    ):
        collision = root / name
...
def test_version_staging_is_regular_and_exact(root: Path) -> None:
    ...
    bundle_version = normal / "release-bundle/0.2.34/VERSION"
    staged_version = normal / "stage-prefix/releases/0.2.34/VERSION"
...
def test_staging_crash_cleanup_and_retry(root: Path) -> None:
    ...
    assert not (retry_home / "release-bundle/0.2.34").exists()
    assert not (retry_home / "stage-prefix/releases/0.2.34").exists()
    assert retry_universal.read_bytes() == universal_before
    retry = _staged_install(retry_home, ROOT)
    retried_version = retry_home / "stage-prefix/releases/0.2.34/VERSION"
...
"""
    ...
    for candidate in (
        crash_home / "release-bundle/0.2.34",
        crash_home / "stage-prefix/releases/0.2.34",
    ):
...
"""
    ...
    assert (
        crash_home / "stage-prefix/releases/0.2.34/VERSION"
...
def test_partial_stage_recovery_mismatches_refuse(root: Path) -> None:
    ...
    for directory in (
        source_home / "release-bundle/0.2.34",
        source_home / "stage-prefix/releases/0.2.34",
    ):
    ...
    bundle = inode_home / "release-bundle/0.2.34"
    ...
    (extra_home / "release-bundle/0.2.34/extra-byte").write_bytes(b"unexpected\n")
    refused(
...
def test_staging_cleanup_preserves_replacements_and_original_refusal(
    ...
    def swap_reserved_bundle(event: str) -> None:
        if event != "after_release_file:VERSION":
            return
        bundle = swapped_home / "release-bundle/0.2.34"
    ...
    assert (swapped_home / "release-bundle/0.2.34/replacement-byte").read_bytes() == (
        b"must remain\n"
    )
    assert (moved_bundle / "VERSION").read_bytes() == source_version.read_bytes()
    assert not (swapped_home / "stage-prefix/releases/0.2.34").exists()
    ...
    def swap_reserved_bundle_to_symlink(event: str) -> None:
        if event != "after_release_file:VERSION":
            return
        bundle = symlink_home / "release-bundle/0.2.34"
    ...
    restored_link = symlink_home / "release-bundle/0.2.34"
    ...
    def swap_staged_subdirectories(event: str) -> None:
        if event != "after_release_file:VERSION":
            return
        (subdirectory_home / "release-bundle/0.2.34/bin").symlink_to(
            foreign_bundle, target_is_directory=True
        )
        (subdirectory_home / "stage-prefix/releases/0.2.34/bin").symlink_to(
