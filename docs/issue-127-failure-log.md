...
| Duplicate JSON members | Python's default object decoding collapsed repeated top-level `error` members and nested `code` members before strict shape validation, allowing the final `pane_not_found` value to verify absence. | Decode every object through a recursive duplicate-member-refusing `object_pairs_hook`; adversarial tests cover both forged payloads. | Synthetic adapter calls only; no canonical, pane, process, or repository mutation. |
