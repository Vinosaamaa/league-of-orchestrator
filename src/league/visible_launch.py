...
def _session_id(agent: Mapping[str, Any]) -> str | None:
    ...
    token_value = tokens.get("identity_thread_id") if isinstance(tokens, Mapping) else None
    if isinstance(token_value, str):
        return token_value
    title = agent.get("terminal_title_stripped", agent.get("terminal_title"))
    if isinstance(title, str) and title.endswith(" | codex"):
        title_value = title.removesuffix(" | codex")
        if THREAD_UUID.fullmatch(title_value) is not None:
            return title_value
    return None
    ...
    def deliver_context(
        ...
        try:
            _, completed = self._command(
                ("herdr", "agent", "prompt", routing_name, context),
        ...
        return {
            ...
            "bytes": len(body),
            "effect_sha256": _sha256(completed.stdout.encode("utf-8")),
        }
    ...
    def launch(self, spec: AssignmentSpec) -> dict[str, Any]:
        ...
        if prior is not None and prior["context_delivery"] is not None:
            ...
            return {
                ...
                "context_delivery": prior["context_delivery"],
                "idempotent": True,
            }
        try:
